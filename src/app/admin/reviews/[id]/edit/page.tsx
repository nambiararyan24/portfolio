...
export default function EditReviewPage() {
  ...
  const fetchReview = async () => {
    try {
      ...
      toast.error(`Failed to fetch review: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`Failed to update review: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Edit Review</h1>
          <p className="text-slate-300 text-lg">Update the client review</p>
        </div>
